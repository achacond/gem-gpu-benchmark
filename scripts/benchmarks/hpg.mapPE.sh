...
echo "==> Mapping $OUT_T1"
profile "./hpg-aligner dna --cpu-threads $num_threads --paired-min-distance 0 --paired-max-distance 1000 -i $local_index_path/HG_index_hpg-aligner_default -f $local_dataset_path/PE.DUMMY.1.fastq -j $local_dataset_path/PE.DUMMY.2.fastq -o $local_results_path/$OUT_T1.sam > $log_path/$OUT_T1.log 2>&1"
