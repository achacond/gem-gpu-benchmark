...
void gt_mapset_specificity_profile() {
  ...
  // Init MAPQ TP/FP vectors
  uint64_t num_first_true=0, num_any_true=0, num_reads = 0, num_maps = 0;
  uint64_t mapq_true[UINT8_MAX];
  uint64_t mapq_false[UINT8_MAX];
  memset(mapq_true,0,UINT8_MAX*sizeof(uint64_t));
  memset(mapq_false,0,UINT8_MAX*sizeof(uint64_t));
  ...
  while (gt_mapset_read_template_get_commom_map(buffered_input_1,buffered_input_2,template_master,template_slave)) {
    ...
    GT_TEMPLATE_ITERATE_MMAP__ATTR(template_slave,mmap_slave,mmap_slave_attr) {
      const uint8_t mapq_score = (__mmap_slave_num_blocks==1) ?
          (input_file_2->file_format==MAP ? mmap_slave[0]->gt_score : mmap_slave[0]->phred_score) :
          (input_file_2->file_format==MAP ? mmap_slave_attr->gt_score : mmap_slave_attr->phred_score);
      if (gt_template_is_mmap_contained_fx(gt_mapset_mmap_cmp,template_master,mmap_slave)) {
        if (position==0) first_tp = true;
        any_tp = true;
        mapq_true[mapq_score]++;
      } else {
        mapq_false[mapq_score]++;
      }
      ++position;
      ++num_maps;
    }
    if (first_tp) ++num_first_true;
    if (any_tp) ++num_any_true;
    ++num_reads;
  }
  ...
  fprintf(stderr,"Num.First.TP %lu (%2.4f%%)\n",num_first_true,(double)num_first_true/(double)num_maps);
  fprintf(stderr,"Num.Any.TP %lu (%2.4f%%)\n",num_any_true,(double)num_any_true/(double)num_maps);
  fprintf(stderr,"ROC (TP/FP)\n");
  int64_t p, acc_true=0, acc_false=0;
  for (p=UINT8_MAX-1;p>=0;--p) {
    acc_true+=mapq_true[p];
    acc_false+=mapq_false[p];
