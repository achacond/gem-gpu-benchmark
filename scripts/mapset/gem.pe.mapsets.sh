...
#SBATCH --job-name="GEM3-GPU-PE-MAPSET"
#SBATCH --exclusive
#SBATCH -x huberman
#SBATCH -x robin
...
#SBATCH --output=../../logs/GEM.mapset.pe.summary.log 
#SBATCH --error=../../logs/GEM.mapset.pe.summary.log
...
profile ./gt.mapset -C specificity-profile --i1 $datasets_sorted_path/$SOURCE.sam.sorted --i2 $sorting_path/$OUT.sam.sorted -p --eq-th 0.20 -o $mapsets_data_path/$OUT.sam 2> $mapsets_info_path/$OUT.sam.sensitivity
echo "Done"
...
OUT="$TAG.$OUT_PREFIX.default.t12.K20"
SOURCE="DATASET.$OUT_PREFIX"
echo -n "==> Mapsets of $SOURCE with $OUT ..."
profile ./gt.mapset -C specificity-profile --i1 $datasets_sorted_path/$SOURCE.sam.sorted --i2 $sorting_path/$OUT.sam.sorted -p --eq-th 0.20 -o $mapsets_data_path/$OUT.sam 2> $mapsets_info_path/$OUT.sam.sensitivity
echo "Done"
